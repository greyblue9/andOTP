    ...
    public class ServiceConnection extends SASocket {
        ...
        public void onReceive(int channelId, byte[] data) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        mConnectionHandler.secureSend(getServiceChannelId(0), message.getBytes());
                    //    mConnectionHandler.send(getServiceChannelId(0), message.getBytes()); //working, already e2e encrypted, but replace by secureSend just in case
